...
Sends one audio file, prints the transcribed text, which engine actually
served it (phowhisper:<model> via Colab, stt-adapter:<id>:<model> for a
published Tier 2 adapter, or phowhisper-local:<model> for the local
fallback), and round-trip latency. Useful to confirm the whole path works
before a demo, without going through the browser mic UI.
